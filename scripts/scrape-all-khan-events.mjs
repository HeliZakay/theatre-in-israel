...
import { fetchShows, scrapeShowEvents, KHAN_THEATRE } from "./lib/hakahn.mjs";
...
async function main() {
  separator();
  console.log(bold(cyan("  Khan Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Khan Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Khan Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Khan Theatre shows in DB.\n`));
