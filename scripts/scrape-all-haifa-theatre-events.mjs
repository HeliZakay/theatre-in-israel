 ...
 *
 * Haifa Theatre is a fixed venue — all events are at תיאטרון חיפה, חיפה.
 *
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrape-all-haifa-theatre-events.mjs                                              # dry-run
 *   node scripts/scrape-all-haifa-theatre-events.mjs --apply                                      # write to DB
 *   node scripts/scrape-all-haifa-theatre-events.mjs --json prisma/data/events-haifa-theatre.json # JSON file
 *   node scripts/scrape-all-haifa-theatre-events.mjs --debug                                      # dump DOM per show
 */
...
async function main() {
  separator();
  console.log(bold(cyan("  Haifa Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Haifa Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Haifa Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Haifa Theatre shows in DB.\n`));
