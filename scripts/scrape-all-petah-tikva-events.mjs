 ...
 *
 * Usage:
 *   node scripts/scrape-all-petah-tikva-events.mjs                              # dry-run
 *   node scripts/scrape-all-petah-tikva-events.mjs --apply                       # write to DB
 *   node scripts/scrape-all-petah-tikva-events.mjs --json prisma/data/events-petah-tikva.json
 *   node scripts/scrape-all-petah-tikva-events.mjs --debug                       # dump DOM per page
 */
...
async function main() {
  separator();
  console.log(bold(cyan("  Petah Tikva Venue Scraper — היכל התרבות פתח תקווה")));
  console.log(
    dim(
  ...
  for (let i = 0; i < matched.length; i++) {
    ...
    try {
      // Each detailUrl = one performance date; scrape JSON-LD from each
      const showEvents = [];
      const seen = new Set();
