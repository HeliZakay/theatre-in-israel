 ...
 *
 * Usage:
 *   node scripts/scrape-all-ganei-tikva-events.mjs                              # dry-run
 *   node scripts/scrape-all-ganei-tikva-events.mjs --apply                       # write to DB
 *   node scripts/scrape-all-ganei-tikva-events.mjs --json prisma/data/events-ganei-tikva.json
 *   node scripts/scrape-all-ganei-tikva-events.mjs --debug                       # dump DOM per page
 */
...
async function main() {
  separator();
  console.log(bold(cyan("  Ganei Tikva Venue Scraper — מרכז הבמה גני תקווה")));
  console.log(
    dim(
