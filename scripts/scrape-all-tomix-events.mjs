...
async function main() {
  separator();
  console.log(bold(cyan("  toMix Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all toMix Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No toMix Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} toMix Theatre shows in DB.\n`));
  ...
  const listingEntries = listings.map(({ title, url }) => ({
    normalised: normaliseForMatch(title),
    url,
  }));
  const listingMap = new Map(listingEntries.map((e) => [e.normalised, e.url]));
  ...
  for (const show of dbShows) {
    ...
    // Exact match
    let url = listingMap.get(key);

    // Fallback: DB title is contained in listing title (handles subtitles)
    if (!url) {
      for (const entry of listingEntries) {
        if (entry.normalised.includes(key) || key.includes(entry.normalised)) {
          url = entry.url;
          break;
        }
      }
