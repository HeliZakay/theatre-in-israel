 ...
 *
 * Tmuna Theatre is a fixed venue — all events are at תיאטרון תמונע, תל אביב.
 *
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrape-all-tmuna-theatre-events.mjs                                                    # dry-run
 *   node scripts/scrape-all-tmuna-theatre-events.mjs --apply                                            # write to DB
 *   node scripts/scrape-all-tmuna-theatre-events.mjs --json prisma/data/events-tmuna-theatre.json       # JSON file
 *   node scripts/scrape-all-tmuna-theatre-events.mjs --debug                                            # dump DOM per show
 */
...
import { fetchShows, scrapeShowEvents, TMUNA_THEATRE } from "./lib/tmuna.mjs";
...
async function main() {
  separator();
  console.log(bold(cyan("  Tmuna Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Tmuna Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Tmuna Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Tmuna Theatre shows in DB.\n`));
