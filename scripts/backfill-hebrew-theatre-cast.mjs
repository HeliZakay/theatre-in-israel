...
async function main() {
  // ── 1. Fetch Hebrew Theatre shows from DB ──
  ...
  try {
    dbShows = await db.prisma.show.findMany({
      where: { theatre: HEBREW_THEATRE, cast: null },
  ...
  if (dbShows.length === 0) {
    console.log(green("All Hebrew Theatre shows already have cast data."));
    return;
  }

  console.error(
    cyan(`Found ${dbShows.length} Hebrew Theatre shows without cast data.\n`),
  );
  ...
  console.error(dim(`Shows page returned ${listings.length} shows.\n`));
  ...
  const lines = [
    "-- Backfill actors-only cast data for Hebrew Theatre (התיאטרון העברי) shows",
    "-- Cast extracted from individual show pages at teatron.org.il",
