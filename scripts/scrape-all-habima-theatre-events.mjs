 ...
 *
 * Habima Theatre is a fixed venue — all events are at תיאטרון הבימה, תל אביב.
 *
...
async function main() {
  separator();
  console.log(bold(cyan("  Habima Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Habima Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Habima Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Habima Theatre shows in DB.\n`));
  ...
  try {
    ...
    listings = await fetchRepertoire(browser);
    console.log(dim(`  Listing returned ${listings.length} shows.\n`));
  } catch (err) {
    console.error(red(`  Failed to fetch repertoire listing: ${err.message}`));
    await browser.close();
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(1);
