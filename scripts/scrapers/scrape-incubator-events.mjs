#!/usr/bin/env node
/**
 * scrape-incubator-events.mjs
 *
 * Scrape performance dates/times from an Incubator Theatre show detail page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-incubator-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-incubator-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-incubator-events.mjs <show-url> --apply     # write to DB
 */

import { launchBrowser } from "../lib/browser.mjs";
import { scrapeShowEvents, INCUBATOR_THEATRE } from "../lib/incubator.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-incubator-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Incubator Theatre Events Scraper")));
  ...
  try {
    ...
    if (debug && result.debugHtml) {
      separator();
      console.log(bold(yellow("  DEBUG: Raw HTML of page")));
      separator();
    ...
    if (apply && result.events.length > 0) {
      ...
      try {
        ...
        const dbShows = await db.prisma.show.findMany({
          where: { theatre: INCUBATOR_THEATRE },
