#!/usr/bin/env node
/**
 * scrape-haifa-theatre-events.mjs
 *
 * Scrape performance dates/times from a single Haifa Theatre show detail page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-haifa-theatre-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-haifa-theatre-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-haifa-theatre-events.mjs <show-url> --apply     # write to DB
 */
...
import { scrapeShowEvents, HAIFA_THEATRE } from "../lib/haifa.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-haifa-theatre-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Haifa Theatre Events Scraper")));
  ...
  try {
    ...
    if (debug && result.debugHtml) {
      separator();
      console.log(bold(yellow("  DEBUG: Raw HTML (first 12 000 chars)")));
      separator();
    ...
    if (apply && result.events.length > 0) {
      ...
      try {
        // Resolve show — Haifa URLs end with /Event/Index/{numericId}
        ...
        const show = await db.prisma.show.findFirst({
          where: {
            ...
            OR: [{ slug: urlId }, { slug: { contains: urlId } }],
          },
        });
        if (!show) {
          console.error(
            red(`  Show not found in DB for URL id: ${urlId}`),
          );
          console.error(
            dim("  Tip: run the all-shows script which matches by title."),
          );
          process.exit(1);
        }
