#!/usr/bin/env node
/**
 * scrape-tomix-events.mjs
 *
 * Scrape performance dates/times from a toMix Theatre show page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-tomix-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-tomix-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-tomix-events.mjs <show-url> --apply     # write to DB
 */

import { launchBrowser } from "../lib/browser.mjs";
import { scrapeShowEvents, TOMIX_THEATRE } from "../lib/tomix.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-tomix-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  toMix Theatre Events Scraper")));
  ...
  try {
    ...
    if (debug && result.debugHtml) {
      separator();
      console.log(bold(yellow("  DEBUG: Raw HTML of eventer page")));
      separator();
