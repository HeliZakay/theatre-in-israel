#!/usr/bin/env node
/**
 * scrape-tmuna-theatre-events.mjs
 *
 * Scrape performance dates/times from a Tmuna Theatre show detail page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-tmuna-theatre-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-tmuna-theatre-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-tmuna-theatre-events.mjs <show-url> --apply     # write to DB
 */

import { launchBrowser } from "../lib/browser.mjs";
import { scrapeShowEvents, TMUNA_THEATRE } from "../lib/tmuna.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-tmuna-theatre-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Tmuna Theatre Events Scraper")));
  ...
  try {
    ...
    if (apply && result.events.length > 0) {
      ...
      try {
        // Look up the show by title (scraped from the page) and theatre
