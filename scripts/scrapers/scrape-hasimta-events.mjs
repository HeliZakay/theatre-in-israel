#!/usr/bin/env node
/**
 * scrape-hasimta-events.mjs
 *
 * Scrape performance dates/times from a Hasimta Theatre show detail page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-hasimta-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-hasimta-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-hasimta-events.mjs <show-url> --apply     # write to DB
 */

import { launchBrowser } from "../lib/browser.mjs";
import { scrapeShowEvents, HASIMTA_THEATRE } from "../lib/hasimta.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-hasimta-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Hasimta Theatre Events Scraper")));
  ...
  try {
    ...
    if (result.events.length === 0) {
      ...
      for (const ev of result.events) {
        ...
        console.log(`  ${ev.date}    ${ev.hour || "??:??"}    ${ticket}`);
      }
    }
    ...
    if (apply && result.events.length > 0) {
      ...
      try {
        ...
        // Fixed venue
        const venueName = "תיאטרון הסימטה";
        const venueCity = "תל אביב-יפו";
