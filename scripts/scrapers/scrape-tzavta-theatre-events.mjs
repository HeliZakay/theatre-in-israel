#!/usr/bin/env node
/**
 * scrape-tzavta-theatre-events.mjs
 *
 * Scrape performance dates/times from a Tzavta Theatre show detail page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-tzavta-theatre-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-tzavta-theatre-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-tzavta-theatre-events.mjs <show-url> --apply     # write to DB
 */
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-tzavta-theatre-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Tzavta Theatre Events Scraper")));
