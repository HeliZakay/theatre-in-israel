#!/usr/bin/env node
/**
 * scrape-beer-sheva-theatre-events.mjs
 *
 ...
 *
 * Usage:
 *   node scripts/scrapers/scrape-beer-sheva-theatre-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-beer-sheva-theatre-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-beer-sheva-theatre-events.mjs <show-url> --apply     # write to DB
 */

import { launchBrowser } from "../lib/browser.mjs";
import { scrapeShowEvents, BEER_SHEVA_THEATRE } from "../lib/beer-sheva.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-beer-sheva-theatre-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Beer Sheva Theatre Events Scraper")));
    ...
    if (apply && result.events.length > 0) {
      ...
      try {
        ...
        const dbShows = await db.prisma.show.findMany({
          where: { theatre: BEER_SHEVA_THEATRE },
