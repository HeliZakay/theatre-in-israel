 ...
 *
 * Usage:
 *   node scripts/scrapers/scrape-cameri-events.mjs <cameri-show-url>             # dry-run
 *   node scripts/scrapers/scrape-cameri-events.mjs <cameri-show-url> --debug     # dump DOM for selector discovery
 *   node scripts/scrapers/scrape-cameri-events.mjs <cameri-show-url> --apply     # write to DB (Venue + Event)
 */
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-cameri-events.mjs <cameri-show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
