#!/usr/bin/env node
/**
 * scrape-khan-events.mjs
 *
 * Scrape performance dates/times from a Khan Theatre show detail page.
 * Dry-run by default — prints scraped dates for review.
 *
 * Usage:
 *   node scripts/scrapers/scrape-khan-events.mjs <show-url>             # dry-run
 *   node scripts/scrapers/scrape-khan-events.mjs <show-url> --debug     # dump DOM
 *   node scripts/scrapers/scrape-khan-events.mjs <show-url> --apply     # write to DB
 */

import { launchBrowser } from "../lib/browser.mjs";
import { scrapeShowEvents, KHAN_THEATRE } from "../lib/hakahn.mjs";
import {
  bold,
  cyan,
  yellow,
...
if (!url) {
  console.error(
    red(
      "Usage: node scripts/scrapers/scrape-khan-events.mjs <show-url> [--debug] [--apply]",
    ),
  );
  process.exit(1);
}
...
async function main() {
  separator();
  console.log(bold(cyan("  Khan Theatre Events Scraper")));
