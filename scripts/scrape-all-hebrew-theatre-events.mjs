...
async function main() {
  separator();
  console.log(bold(cyan("  Hebrew Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Hebrew Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Hebrew Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Hebrew Theatre shows in DB.\n`));
  ...
  for (let i = 0; i < matched.length; i++) {
    ...
    try {
      ...
      if (result.events.length === 0) {
        ...
        // Warn if many events have missing hours
        ...
        if (!apply || debug) {
          for (const ev of result.events) {
            const venue = ev.venueName ? bidi(ev.venueName) : "(unknown)";
        ...
        if (jsonPath) {
          for (const ev of result.events) {
            collectedEvents.push({
              ...
              venueCity:
                ev.venueCity ||
                resolveVenueCity(ev.venueName || "התיאטרון העברי"),
            });
          }
        }
