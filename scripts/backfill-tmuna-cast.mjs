...
async function main() {
  ...
  try {
    dbShows = await db.prisma.show.findMany({
      where: { theatre: TMUNA_THEATRE, cast: null },
  ...
  if (dbShows.length === 0) {
    console.log(green("All Tmuna Theatre shows already have cast data."));
    return;
  }

  console.error(
    cyan(`Found ${dbShows.length} Tmuna shows without cast data.\n`),
  );
