...
<head>
  ...
  <title>הצגות חסרות - תיאטרון הבימה</title>
  <style>
...
<body>
  <div class="top-bar">
    <h1>🎭 הצגות חסרות - תיאטרון הבימה</h1>
...
function saveAndOpenHtml(results) {
  ...
  const outPath = path.join(rootDir, "missing-habima-shows.html");
...
try {
  // 1. DB lookup
  const existingSet = await fetchExistingTitles(HABIMA_THEATRE);
  ...
  // 2. Scrape repertoire page
  const browser = await launchBrowser();

  let allShows;
  try {
    allShows = await fetchRepertoire(browser);
  } catch (err) {
    await browser.close();
    throw err;
  }
  ...
  if (missingShows.length === 0) {
    if (jsonMode) {
      ...
      console.log("\n🎭  All Habima shows are already in the database. ✅\n");
      console.log(
