...
async function main() {
  ...
  try {
    dbShows = await db.prisma.show.findMany({
      where: { theatre: GESHER_THEATRE, cast: null },
  ...
  if (dbShows.length === 0) {
    console.log(green("All Gesher Theatre shows already have cast data."));
    return;
  }

  console.error(
    cyan(`Found ${dbShows.length} Gesher shows without cast data.\n`),
  );
