...
import { HAIFA_THEATRE, fetchShows, scrapeShowDetails } from "./lib/haifa.mjs";
...
async function main() {
  ...
  try {
    dbShows = await db.prisma.show.findMany({
      where: { theatre: HAIFA_THEATRE, cast: null },
  ...
  if (dbShows.length === 0) {
    console.log(green("All Haifa Theatre shows already have cast data."));
    return;
  }

  console.error(
    cyan(`Found ${dbShows.length} Haifa shows without cast data.\n`),
  );
  ...
  console.error(dim(`Schedule page returned ${listings.length} shows.\n`));
