...
import { fetchShows, scrapeShowEvents, GESHER_THEATRE } from "./lib/gesher.mjs";
...
async function main() {
  separator();
  console.log(bold(cyan("  Gesher Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Gesher Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Gesher Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Gesher Theatre shows in DB.\n`));
