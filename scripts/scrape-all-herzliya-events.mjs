 ...
 *
 * Usage:
 *   node scripts/scrape-all-herzliya-events.mjs                              # dry-run
 *   node scripts/scrape-all-herzliya-events.mjs --apply                       # write to DB
 *   node scripts/scrape-all-herzliya-events.mjs --json prisma/data/events-herzliya.json
 *   node scripts/scrape-all-herzliya-events.mjs --debug                       # dump DOM per page
 */
...
async function main() {
  separator();
  console.log(bold(cyan("  Herzliya Venue Scraper — היכל אמנויות הבמה הרצליה")));
  console.log(
    dim(
  ...
  if (debug) {
    ...
    for (const item of listings) {
      console.log(dim(`    ${bidi(item.title)} → ${item.detailUrl}`));
    }
    console.log("");
  }
