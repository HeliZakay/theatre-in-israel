...
async function main() {
  ...
  try {
    dbShows = await db.prisma.show.findMany({
      where: { theatre: HABIMA_THEATRE, cast: null },
  ...
  if (dbShows.length === 0) {
    console.log(green("All Habima Theatre shows already have cast data."));
    return;
  }

  console.error(
    cyan(`Found ${dbShows.length} Habima shows without cast data.\n`),
  );

  // ── 2. Scrape listing page for title→URL mapping ──
  const browser = await launchBrowser();
  const listings = await fetchRepertoire(browser);

  console.error(dim(`Repertoire page returned ${listings.length} shows.\n`));
  ...
  const lines = [
    "-- Backfill actors-only cast data for Habima Theatre (תיאטרון הבימה) shows",
    "-- Cast extracted from individual show pages at habima.co.il",
