 ...
 *   node scripts/scrape-all-lessin-events.mjs --json prisma/data/events-lessin.json  # write JSON file
...
async function main() {
  separator();
  console.log(bold(cyan("  Lessin Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Lessin shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Lessin shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Lessin shows in DB.\n`));
