 ...
 *
 * Beer Sheva Theatre is a fixed venue — all events are at תיאטרון באר שבע, באר שבע.
 *
...
import { fetchShows, scrapeShowEvents, BEER_SHEVA_THEATRE } from "./lib/beer-sheva.mjs";
...
async function main() {
  separator();
  console.log(bold(cyan("  Beer Sheva Theatre Events Scraper — All Shows")));
  console.log(
    dim(
  ...
  // ── 1. Connect to DB and query all Beer Sheva Theatre shows ──
  ...
  if (dbShows.length === 0) {
    console.log(yellow("  No Beer Sheva Theatre shows found in DB."));
    await db.prisma.$disconnect();
    await db.pool.end();
    process.exit(0);
  }

  console.log(dim(`\n  Found ${dbShows.length} Beer Sheva Theatre shows in DB.\n`));
